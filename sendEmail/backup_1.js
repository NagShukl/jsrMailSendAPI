...
class SendEmail {
    ...
    handleSendMailRequest(req, res) {
        ...
        req.on('data', (reqData) => {
            ...
            const response = this.performMailSendAction(mailData);
            res.write(response + 'Jai Shri Ram!' + req.method + ' : ' + req.url + ' : ' + req.headers['jsr']);
            res.end();
        });
        return;

    }
    performMailSendAction(mailData, providerNumber) {
        ...
        selectedProvider.sendMail(mailData).then(data => {
            ...
            this.performMailSendAction(mailData, ++providerNumber);
        });
        console.log('========');
